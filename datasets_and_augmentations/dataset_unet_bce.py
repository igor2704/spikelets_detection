...
class SpikeletsDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        transforms = get_transforms(self.cfg_aug)
        
        crop_img, crop_mask = get_random_crops(img, segmentation_mask, point_mask, random=self.train)
        distance = get_radius(crop_mask)
        
        if self.train:
            augmentations =  transforms['train'](image=crop_img, mask=crop_mask)
        else:
            augmentations = transforms['test'](image=crop_img, mask=crop_mask)
